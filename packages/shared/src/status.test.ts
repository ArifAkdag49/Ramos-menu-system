...
describe('parseStatus — raw onaltılık (Buffer kullanmadan)', () => {
  it('baytlar küçük harf, iki haneli hex olarak dizilir', () => {
    expect(parseStatus(Uint8Array.from([0x12, 0x0a, 0xff])).raw).toBe('120aff');
    expect(parseStatus(new Uint8Array()).raw).toBe('');
  });
});
