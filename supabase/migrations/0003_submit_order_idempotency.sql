-- 0003 (idempotency) — submit_order: masa satırı kilidi idempotency kontrolünden önce alınır (R36, spec §13).
-- Aynı order_id ile eşzamanlı iki istekte ikincisi 23505 (orders_pkey) yerine mevcut sonucu (duplicate) alır.
-- Gövdenin geri kalanı 0003_submit_order.sql ile aynıdır; yeni hata anahtarı yoktur.
...
begin
  ...
  -- R36: masa satırı idempotency kontrolünden ÖNCE kilitlenir. Aynı order_id ile eşzamanlı ikinci istek
  -- burada ilkinin commit'ini bekler, sonra aşağıdaki kontrolde onun siparişini görür (23505 yerine duplicate).
  ...
  -- FOUND artık sipariş sorgusuna ait; masa bulunamadıysa v_table'ın tüm alanları null'dır.
