...
### M4 — Mutfak (Plan 3)
- [~] Görev 16 — Mutfak ekranı (KDS) · tasarım kapısı — `bba349e`, `6bed015` — apps/web 27 dosya / 145 test ✓, `tsc -b` + `eslint .` + `vite build` temiz; inceleme 3 Important (çift realtime aboneliği, `Elapsed` bileşeninin yeniden yazılması, mutasyonlarda hata geri bildirimi yok) + 1 minor buldu, düzeltme turu 1 uygulandı, kapsamlı yeniden inceleme sürüyor. M4 tasarım kapısı açık
