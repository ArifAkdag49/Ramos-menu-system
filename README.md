...
| `apps/mobile` | Android uygulaması (Capacitor, paket `com.arxdigital.ramos`): canlı siteyi açar, FCM bildirimleri, `RamosPrinter` eklentisi + arka plan yazıcı istasyonu (`StationService`) + ağda yazıcı bulma (`PrinterDiscovery`: TCP taraması + SNMP yayını `SnmpProbe` + Bonjour `NsdProbe`; soketler Wi-Fi/Ethernet'e bağlanır: `LanNetworks`; istasyon IP değişince yazıcıyı kendisi yeniden bulur), imzalı APK betiği `build-apk.ps1` (`-Bundled`: site gömülü) |
