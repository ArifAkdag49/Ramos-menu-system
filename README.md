...
| `apps/mobile` | Android uygulaması (Capacitor, paket `com.arxdigital.ramos`): canlı siteyi açar, FCM bildirimleri, `RamosPrinter` eklentisi + arka plan yazıcı istasyonu (`StationService`) + ağda yazıcı bulma (`PrinterDiscovery`, soketler Wi-Fi/Ethernet'e bağlanır: `LanNetworks`), imzalı APK betiği `build-apk.ps1` |
