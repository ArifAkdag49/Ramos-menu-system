...
const SECTIONS: NavItem[] = [
  ...
  {
    to: '/admin/tables',
    labelKey: 'admin.tables.title',
    icon: <LayoutGrid aria-hidden size={20} />,
  },
  {
    to: '/admin/orders',
  ...
  return (
    <div className="min-h-dvh text-sm lg:pl-64">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col border-r border-border bg-surface lg:flex">
        <div className="flex h-[var(--header-h)] shrink-0 items-center px-5 text-lg font-semibold">
          RAMO&apos;S
        </div>
        <AdminNav />
      </aside>
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={() => void signOut()}
        className={clsx(
          'mt-6 flex min-h-12 items-center gap-3 rounded-control px-3 font-medium text-muted',
          'transition-colors duration-150 ease-out hover:bg-surface-2 hover:text-text',
